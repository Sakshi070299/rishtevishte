...
export default function ManagerPrintPage() {
  ...
  const rangeValid  = fromDate && toDate && new Date(fromDate) <= new Date(toDate);
  ...
  const handleLoad = useCallback(async () => {
    ...
    try {
      const data = await teamApi.printRegistrations(fromDate, toDate) as Profile[];
      setProfiles(data);
      setLoaded(true);
      if (data.length === 0) {
        toast.info('No registrations found for this date range');
      } else {
        toast.success(`Loaded ${data.length} registration${data.length !== 1 ? 's' : ''}`);
      }
    } catch (err: unknown) {
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <p className="text-sm text-amber-800 leading-relaxed">
            <span className="font-semibold">Restriction:</span> Only registrations from the
            last 2 weeks are printable. Date ranges exceeding 14 days will be rejected.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <h2 className="font-semibold text-gray-900 mb-4 flex items-center gap-2 text-sm">
            <CalendarDays size={16} style={{ color: EMERALD }} />
            Select Date Range
          </h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-900 focus:outline-none transition-colors"
                onFocus={(e) => {
          ...
          {rangeExceed && (
            <div className="flex items-center gap-2 mb-4 p-3 rounded-lg bg-red-50 border border-red-200">
              ...
              <p className="text-xs text-red-700 font-medium">
                Selected range is {diffDays(fromDate, toDate)} days — exceeds the 14-day limit.
              </p>
            </div>
          )}
          ...
          <button
            ...
            onMouseEnter={(e) => {
              if (!loading && rangeValid && !rangeExceed) {
        ...
        {error && !loading && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center">
            ...
            <p className="font-semibold text-red-700 mb-1">Failed to load registrations</p>
        ...
        {loaded && !error && (
          <div id="print-table-section">
            ...
            <div className="print:block">
              <h2 className="text-lg font-bold hidden print:block" style={{ color: '#064e3b' }}>
                TheMarriageHome.com — Registrations
              </h2>
              <p className="print-meta hidden print:block text-xs text-gray-500 mb-3">
                Date range: {formatDate(fromDate)} to {formatDate(toDate)} &bull; Total:{' '}
                {profiles.length} registrations
              </p>
            </div>
            ...
            <div className="flex items-center justify-between mb-3 print:hidden">
              <p className="text-sm font-medium text-gray-700">
                <span className="font-bold text-lg" style={{ color: '#064e3b' }}>
                  {profiles.length}
                </span>{' '}
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              {profiles.length === 0 ? (
                <div className="px-5 py-16 text-center">
                  ...
                  <p className="font-semibold text-gray-500">No registrations found</p>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      ...
                      >
                        {['Reg #', 'Full Name', 'Gender', 'Father Name', 'Phone', 'Registered On'].map((h) => (
                          <th
                            key={h}
                    ...
                    <tbody>
                      {profiles.map((p, idx) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5 text-gray-700 whitespace-nowrap font-mono text-xs">
                            {p.guardianPhone}
                          </td>
