...
export default function MyActivityPage() {
  ...
  // Preserve a predictable order: known actions first, then unknowns
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-bold text-blue-900">My Activity Log</h1>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        {/* Days filter toggle */}
        <div className="inline-flex items-center gap-1 p-1 rounded-lg border border-gray-200 bg-white shadow-sm">
          {DAY_OPTIONS.map((d) => (
            <button
              key={d}
              onClick={() => setDays(d)}
        ...
        {!loading && !error && (
          <p className="text-sm text-gray-500 sm:ml-auto">
            <span className="font-bold text-blue-900 text-base">{entries.length}</span>{' '}
              ...
              <tbody>
                {entries.map((entry, idx) => (
                  <tr
                    ...
                    style={{
                      backgroundColor: idx % 2 === 0 ? '#fff' : 'rgba(239,246,255,0.30)',
                    }}
                  >
                    {/* Date / Time */}
